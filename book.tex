% LaTeX source for ``Think Python: Pensare da informatico''
...
\newcommand{\thetitle}{Think Python: Pensare da informatico}
...
\item Lokesh Kumar let me know about some broken links and some
...
\chapter{Imparare a programmare}
...
Il linguaggio di programmazione che imparerai è il Python. Python è un esempio di linguaggio di {\bf alto livello}; altri linguaggi di alto livello di cui puoi aver sentito parlare sono il C, il C++, il Perl ed il Java.

Come puoi immaginare sentendo la definizione ``linguaggio di alto livello'' esistono anche linguaggi di {\bf basso livello}, talvolta chiamati ``linguaggi macchina'' o ``linguaggi assembly''. In modo non del tutto corretto si può affermare che i computer possono eseguire soltanto programmi scritti in linguaggi di basso livello: i programmi scritti in un linguaggio di alto livello devono essere elaborati prima di poter essere eseguiti. Questo processo di elaborazione impiega del tempo e rappresenta un piccolo svantaggio dei linguaggi di alto livello.
...
La figura~\ref{fig.interpret} mostra la struttura di un interprete.
\index{source code}
\index{object code}
\index{executable}
...
\caption{L'interprete elabora il programma un po' alla volta, alternando la lettura delle istruzioni all'esecuzione dei comandi che le istruzioni descrivono.}
\label{fig.interpret}
\end{figure}
...
La figura~\ref{fig.compile} mostra la struttura di un compilatore.

\begin{figure}
\centerline
{\includegraphics[scale=0.9]{figs/compile.pdf}}
\caption{Il compilatore traduce il codice sorgente in codice oggetto, il quale è eseguito da un processore hardware.}
\label{fig.compile}
\end{figure}

Python è considerato un linguaggio interpretato perché i programmi Python sono eseguiti da un interprete. Ci sono due modi di usare l'interprete: in {\bf modo interattivo} o in {\bf modo script}. In modo ``interattivo'' si scrivono i programmi Python una riga alla volta: dopo avere scritto una riga di codice alla pressione di Invio (o Enter, a seconda della tastiera) l'interprete la analizza subito ed elabora immediatamente il risultato, eventualmente stampandolo a video:
...
La terza riga inizia con\verb">>>": questa è l'indicazione (chiamata ``prompt'') che l'interprete usa per indicare la sua disponibilità ad accettare comandi. Noi
abbiamo inserito {\tt 1 + 1} e l'interprete ha risposto con {\tt 2}.
\index{prompt}

In alternativa alla riga di comando si può scrivere un programma in un file (detto {\bf script}) ed usare l'interprete per eseguire il contenuto del file.
Per convenzione, i file contenenti programmi Python hanno nomi che terminano con {\tt .py}.
\index{script}

Per eseguire il programma dobbiamo dire all'interprete il nome dello script. Se hai uno script chiamato {\tt pippo.py} e stai lavorando in una finestra di comando UNIX, scriverai {\tt python pippo.py}. In altri ambienti di sviluppo i dettagli dell'esecuzione dei programmi possono essere diversi. Puoi trovare le istruzioni
per il tuo ambiente di sviluppo sul sito di Python \url{http://python.org}.
\index{testing!interactive mode}

Lavorare in modo interattivo è comodo per testare piccole parti di codice perché
puoi scriverle ed eseguirle immediatamente. Ma per qualsiasi codice più lungo di
un paio di righe, dovresti salvare il tuo codice come script così da poterlo
modificare ed eseguire in futuro.

\section{Cos'è un programma?}

Un {\bf programma} è una sequenza di istruzioni che specificano come effettuare una elaborazione. L'elaborazione può essere sia di tipo matematico (per esempio la soluzione di un sistema di equazioni o il calcolo delle radici di un polinomio) che simbolico (per esempio la ricerca e sostituzione di un testo in un documento oppure, stranamente, la compilazione di un programma).
\index{programma}
...
\item[input:] ricezione di dati da tastiera, da file o da altro dispositivo.

\item[output:] scrittura di dati su video, su file o trasmissione ad altro dispositivo.

\item[matematiche:] esecuzione di semplici operazioni matematiche, quali l'addizione e la moltiplicazione.

\item[condizionali:] controllo di alcune condizioni ed esecuzione della sequenza di istruzioni appropriata.
...
\section{Cos'è il debug?}
\index{debugging}
\index{bug}

La programmazione è un processo complesso e dato che esso è fatto da esseri umani spesso comporta errori. Per ragioni bizzarre gli errori di programmazione sono chiamati {\bf bug} ed il processo della loro ricerca e correzione è chiamato {\bf debug}.
{\bf debugging}.
\index{debugging}
\index{bug}
...
Python può eseguire un programma solo se il programma è sintatticamente corretto, altrimenti l'elaborazione fallisce e l'interprete ritorna un messaggio d'errore. La {\bf sintassi} si riferisce alla struttura di un programma e alle regole concernenti la sua struttura. Per esempio, le parentesi devono essere bilanciate (ogni parentesi aperta ne comporta una chiusa), qundi {\tt (1 +2)} è un'istruzione valida, mentre {\tt 8)} è un {\bf errore di sintassi}.
\index{syntax} 
...
\section{Linguaggi formali e naturali}
...
I {\bf linguaggi formali} sono linguaggi progettati per specifiche applicazioni.
Per fare qualche esempio, la notazione matematica è un linguaggio formale particolarmente indicato ad esprimere relazioni tra numeri e simboli; i chimici usano un linguaggio formale per rappresentare la struttura delle molecole; cosa più importante dal nostro punto di vista,

\begin{quote}
{\bf I linguaggi di programmazione sono linguaggi formali che sono stati progettati per esprimere elaborazioni.}
\end{quote}

I linguaggi formali tendono ad essere piuttosto rigidi per quanto riguarda la sintassi: $3 + 3 = 6$ è una dichiarazione matematica sintatticamente corretta, mentre $3 + = 3 \mbox{\$} 6$ non lo è.
$H_2O$ è un simbolo chimico sintatticamente corretto contrariamente a $_2Zz$.

Le regole sintattiche si possono dividere in due categorie: la prima riguarda i {\bf token}, la seconda la struttura. I token sono gli elementi di base del linguaggio (quali possono essere le parole in letteratura, i numeri in matematica e gli elementi chimici in chimica). Uno dei problemi con $3 + = 3 \mbox{\$} 6$ è che \( \$ \) non è un token valido in matematica; $_2Zz$ non è valido perché nessun elemento chimico è identificato dal simbolo $Zz$.
\index{token}
\index{structure}

Il secondo tipo di regola riguarda la struttura della dichiarazione, cioè il modo in cui i token sono disposti. La dichiarazione $3 + = 3$ è strutturalmente non valida perché benchè $+$ e $=$ siano token legittimi, un segno = non può essere posto immediatamente dopo un segno +. Allo stesso modo l'indice nelle formule chimiche deve essere indicato dopo il simbolo dell'elementi chimico, non prima, e quindi l'espressione $2Zz$ non è valida.

\begin{exercise}

Scrivi quella che può sembrare una frase in italiano con dei token non riconoscibili. Poi scrivi un'altra frase con tutti i token validi ma con una struttura non valida.

\end{exercise}

Quando leggi una frase in italiano o una dichiarazione in un linguaggio formale devi capire quale sia la struttura della dichiarazione. Questo processo (chiamato {\bf parsing}) in un linguaggio naturale viene realizzato in modo inconscio e spesso non ci si rende conto della sua intrinseca complessità.
\index{parse}
...
\item[Ambiguità:] i linguaggi naturali ne sono pieni ed il significato viene ottenuto anche grazie ad indizi ricavati dal contesto. I linguaggi formali sono progettati per essere completamente non ambigui e ciò significa che ciascuna dichiarazione ha esattamente un significato, indipendente dal contesto.

\item[Ridondanza:] per evitare l'ambiguità e ridurre le incomprensioni i linguaggi naturali impiegano molta ridondanza. I linguaggi formali sono meno ridondanti e più concisi.

\item[Letteralità:] i linguaggi naturali fanno uso di paragoni e metafore, e possiamo parlare in termini astratti intuendo immediatamente che ciò che sentiamo ha un significato simbolico. I linguaggi formali invece esprimono esattamente ciò che dicono.
...
\item[Poesia:] le parole sono usate tanto per il loro suono che per il loro significato, e la poesia nel suo complesso crea un effetto o una risposta emotiva. L'ambiguità è non solo frequente, ma spesso addirittura cercata.

\item[Prosa:] il significato delle parole è estremamente importante, con la struttura che contribuisce a fornire maggior significato. La prosa può essere soggetta ad analisi più facilmente della poesia, ma può risultare ancora ambigua.

\item[Programmi:] il significato di un programma per computer è non ambiguo e assolutamente letterale, può essere compreso nella sua interezza con l'analisi dei token e della struttura.

\end{description}

Qui sono esposti alcuni suggerimenti per la lettura di programmi e di altri linguaggi formali.  Prima di tutto, ricorda che i linguaggi formali sono molto più ricchi di significato dei linguaggi naturali, così è necessario più tempo per leggerli e comprenderli.  Inoltre, la struttura dei linguaggi formali è molto importante e solitamente non è una buona idea leggerli dall'alto in basso, da sinistra a destra, come avviene per un testo letterario: impara ad analizzare il programma nella tua testa, identificandone i token ed interpretandone la struttura. Infine, i dettagli sono importanti: piccole cose come errori di ortografia e cattiva punteggiatura sono spesso trascurabili nei linguaggi naturali, ma possono fare una gran differenza in quelli formali.
...
\end{verbatim}
%
Questo è un esempio di {\bf istruzione di stampa}, che in effetti non stampa nulla su carta limitandosi invece a scrivere un valore sullo schermo. In questo caso ciò che viene ``stampato'' sono le parole
...
In Python 3, la sintassi per stampare un valore a video è leggermente diversa:

\begin{verbatim}
print('Hello, World!')
\end{verbatim}
%
Le parentesi indicano che {\tt print} è una funzione. Incontreremo le funzioni nel Captiolo~\ref{funcchap}.
\index{function} \index{print function} \index{Python 3}

Per il resto di questo libro, userò l'{\em istruzione} print. Se sai usando Python 3, dovrai usare la funzione. Ma a parte questo, ci sono davvero poche differenze di cui preoccuparsi.
\section{Debugging}
\index{debugging}

È una buona idea leggere questo libro di fronte a un computer così da poter provare gli esempi durante la lettura. Puoi eseguire la maggior parte degli esempi in modo interrativo, ma se salvi il codice in uno script ti sarà più facile provare variazioni diverse.

Quando provi cosa nuove, prova a commettere errori. Per esempio, nel programma ``Hello, world!'', cosa succede se ometti una delle virgolette? Cosa succede se le ometti entrambe? Cosa succede se scrivi {\tt print} sbagliato?
\index{error message}

Questo tipo di esperimenti ti aiuta a ricordare ciò che hai letto; aiuta anche col debug, perché impari a riconoscere il significato del messaggio d'errore.
È preferibile fare errori adesso e volontariamente che dopo ed accidentalmente.

Programmare, ed in particolare eseguire il debug, alle volte suscita forti emozioni. Se stai lottando contro un bug difficile, potresti arrabbiarti, abbatterti o sentirti mortificato.

Ci sono studi che evidenziano come le persone sia naturalmente disposte a rispondere ai computer come se fossero persone reali. Quando funzionano, pensiamo ad essi come compagni di squadra. E quando sono testardi o sgarbati, rispondiamo loro allo stesso modo a cui risponderemmo a persone sgarbate o testarde (Reeves and Nass, {\it The Media
    Equation: How People Treat Computers, Television, and New Media
    Like Real People and Places}).
\index{debugging!emotional response}
\index{emotional debugging}

Preparsi per queste reazioni più aiutarti ad affrontarle.
Un approccio è pensare al computer come ad un dipendente con specifici punti di forza, quali velocità ed accuratezza, e particolari punti deboli, quali mancanza d'empatia ed incapacità di catturare la visione d'insieme.

Il tuo lavoro è essere un buon capo: trovare modi di sfruttare i punti di forza e minimizzare i punti deboli. E trovare modi di usare le tue emozioni per relazionarsi col problema, senza lasciare che le tue reazioni interferiscano con la tua capacità di lavorare efficientemente.

Imparare il debug può essere frustrante, ma è una tecnica preziosa che è utile per molte altre attività oltre alla programmazione. Alla fine di ogni capitolo ci sarà una sezione dedicata al debug, come questa, con i miei pensieri a proposito. Spero che ti siano d'aiuto!

\section{Glossario}

\begin{description}

\item[Soluzione di problemi:]  il processo di formulare un problema, trovare una soluzione ed esprimerla.
\index{problem solving}

\item[Linguaggio ad alto livello:]  un linguaggio di programmazione tipo Python che è progettato per essere facilmente leggibile e utilizzabile dagli esseri umani..
\index{high-level language}

\item[Linguaggio di basso livello:]  un linguaggio di programmazione che è progettato per essere facilmente eseguibile da un computer; è anche chiamato ``linguaggio macchina'' o ``linguaggio assembly''.
\index{low-level language}

\item[Portabilità:]  caratteristica di un programma di poter essere eseguito su computer di tipo diverso.
\index{portability}

\item[Interpretare:]  eseguire un programma scritto in un linguaggio di alto livello traducendolo ed eseguendolo immediatamente, una linea alla volta..
\index{interpret}

\item[Compilare:]  tradurre un programma scritto in un linguaggio di alto livello in un programma di basso livello come preparazione alla successiva esecuzione.
\index{compile}

\item[Codice sorgente:]  un programma di alto livello prima di essere compilato.
\index{source code}
...
\item[Prompt:]  caratteri mostrati dall'interprete per indicare che è pronto ad accettare input dall'utente.
\index{prompt}

\item[Script:]  programma memorizzato in un file, solitamente destinato ad essere interpretato..
\index{script}

\item[modo interrativa:] usare l'interprete Python interpreter scrivendo comandi ed espressioni al prompt.
\index{interactive mode}

\item[modo script:] usare l'interprete Python per leggere ed eseguire istruzioni contenute in uno script.
\index{script mode}

\item[Programma:] serie di istruzioni che specificano come effettuare un'elaborazione.
\index{program}

\item[Algoritmo:]  processo generale usato per risolvere una particolare categoria di problemi.
\index{algorithm}

\item[Bug:]  errore in un programma (detto anche "baco")..
\index{bug}

\item[Debug:]  processo di ricerca e di rimozione di ciascuno dei tre tipi di errori di programmazione.
\index{debugging}

\item[Sintassi:]  struttura di un programma..
\index{syntax}

\item[Errore di sintassi:]  errore in un programma che rende impossibile la continuazione dell'analisi del codice (il programma non può quindi essere interpretato interamente o compilato).

\item[Eccezione:]  error rilevato durante l'esecuzione del programma. Altro nome per errore di esecuzione.
\index{exception}

\item[Semantica:]  significato di un programma..
\index{semantics}

\item[Errore di semantica:]   errore che risulta nell'esecuzionde di qualcosa di diverso da quello che il programmatore intendeva.
\index{semantic error}

\itemLinguaggio naturale:]  ognuno dei linguaggi parlati evoluti nel tempo.
\index{natural language}

\item[Linguaggio formale:]  ognuno dei linguaggi che sono stati progettati per scopi specifici, quali la rappresentazione di idee matematiche o programmi per computer (tutti i linguaggi per computer sono linguaggi formali).
\index{formal language}

\item[Token:]  uno degli elementi di base della struttura sintattica di un programma, analogo alla parola nei linguaggi naturali.
\index{token}

\item[Parsing:]  esame e analisi della struttura sintattica di un programma..
\index{parse}

\item[Istruzione di stampa:]  istruzione che ordina all'interprete Python di scrivere un valore sullo schermo.
\index{print statement}
\index{statement!print}
...
Usa un browser web per andare al sito di Python \url{http://python.org}.
Questa pagina contiene informazioni e link ad altre pagine relative a Python, e fornisce l'abilità di cercare all'interno della documenzione.

Per esempio, se inserisci {\tt print} nel cmapo di ricerca, il primo link che apparirà è la documentazion dell'istruzione {\tt print}. A questo punto, non tutto avrà senso per te, ma è bene sapere dove trovare queste informazioni.
\index{documentation}
\index{python.org}
...
Avvia l'interprete Python e digita {\tt help()} per avviare l'utilià di aiuto in linea. Oppure puoi digitare \verb"help('print')" per ottenere informazioni sull'istruzione {\tt print}.

Se questo esempio non funziona, potresti aver bisogno di installare la documentazione Python aggiuntiva o impostare una variabile d'ambiente; i dettagli dipendono dal tuo sistema operativo e dalla tua version di Python.
\index{help utility}
...
Avvia l'interprete Python ed usalo come una calcolatrice.
La sintassi che Python usa per le operazioni matematiche è quasi la stessa di quella usata per la notazione matematica standard.  Per esempio, i simboli {\tt +}, {\tt -} e {\tt /} denotano addizione, sottrazione e divisione, come ti aspetteresti. Il simbolo per la moltiplicazione è {\tt *}.

Se corri in una gara di 10 kilometri per 43 minuti e 30 secondi, qual'è il tuo tempo medio per miglia? Qual'è la tua velocità media in miglia per ora? (Suggerimento: ci sono 1.61 kilometri in un miglio).
\index{calculator}
\index{running pace}
...
è una delle cose fondamentali manipolate da un programma, come lo sono una lettera dell'alfabeto nella scrittura o un numero in matematica. I valori che abbiamo visto finora sono {\tt 1}, {\tt 2} e \verb"'Hello, World!'".

Questi valori appartengono a {\bf tipi} diversi: {\tt 2} è un intero, e \verb"'Hello, World!'" è una {\bf stringa}, così chiamata perché contiene una serie (o ``stringa'') di caratteri. L'interprete può identificare le stringhe perché sono racchiuse da virgolette.
...
\end{verbatim}
%
Ovviamente le stringhe appartengono al tipo {\tt str} e gli interi al tipo {\tt int}. È meno intuitivo il fatto che i numeri con il punto decimale appartengano al tipo {\tt float}: questi numeri sono rappresentati in un formato chiamato {\bf virgola mobile} o {\bf floating-point}.
...
\end{verbatim}
%
Cosa dire di numeri come \verb"'17'" e \verb"'3.2'"? Sembrano effettivamente dei numeri, ma sono racchiusi tra virgolette e questo sicuramente significa qualcosa. Infatti non siamo in presenza di numeri ma di stringhe:
...
Quando scrivi numeri grandi puoi essere tentato di usare dei punti per delimitare i gruppi di tre cifre, come in {\tt 1.000.000}. Questa in effetti non è una cosa consentita in Python ed il valore numerico in questo caso non è valido. È invece corretta una scrittura del tipo
\begin{verbatim}
>>> 1,000,000
(1, 0, 0)
\end{verbatim}
%
...anche se probabilmente questo risultato non è quello che ci si aspettava! Python interpreta 1,000,000 come una lista di tre valori da stampare (1, 0 e 0). Questo è il primo esempio di errore semantico: il codice viene eseguito senza produrre alcun messaggio d'errore, ma non sta facendo la cossa ``giusta''. Ricordati di non inserire virgole nei tuoi interi.
\index{semantic error}
\index{error!semantic}
\index{error message}
...
Una delle caratteristiche più potenti in un linguaggio di programmazione è la capacità di manipolare {\bf variabili}. Una variabile è un nome che si riferisce ad un valore.

L'{\bf istruzione di assegnazione} crea nuove variabili e assegna loro un valore:
...
\end{verbatim}
%
Questo esempio effettua tre assegnazioni. La prima assegna una stringa ad una nuova variabile chiamata {\tt messaggio}. La seconda assegna l'intero {\tt 17} alla variabile {\tt n} e la terza assegna il valore (approsimato) di $\pi greco$ alla variabile {\tt pi}.
\index{state diagram}
\index{diagram!state}

Un modo comune di rappresentare le variabili sul carta è scriverne il nome con una freccia che punta al valore della variabile. Questo tipo di figura è chiamato {\bf diagramma di stato} perché mostra lo stato in cui si trova la variabile (immagina come se fosse lo ``stato mentale'' della variabile). Figura~\ref{fig.state2} mostra il risultato dell'esempio precedente:
...
Se scrivi un intero che è preceduto da uno zero, potresti ottenere uno strano errore:
\begin{verbatim}
>>> cap = 02492
              ^
...
Altri numeri sembrano funzionari, ma i risultati sono alquanti bizzarri:
\begin{verbatim}
>>> cap = 02132
>>> cap
1114
...
Puoi capire cosa sta succedendo? Suggerimento: mostra i valori {\tt 01}, {\tt 010}, {\tt 0100} e {\tt 01000}.
\index{octal}
...
\section{Nomi delle variabili e parole riservate}
\index{keyword}

I programmatori generalmente scelgono dei nomi significativi per le loro variabili, documentando così a che cosa servono.

I nomi delle variabili possono essere lunghi quanto si desidera e possono contenere sia lettere che numeri, ma devono sempre iniziare con una lettera. È legale usare sia lettere maiuscole che minuscole, ma è preferibile che il noome della variabile inizi con una minuscola (vedremo dopo il perché).

Il carattere di sottolineatura (\verb"_") può far parte di un nome ed è spesso usato in nomi di variabile composti da più parole (per esempio \verb"il_mio_nome" e \verb"prezzo_del_the".
...
\end{verbatim}
%
{\tt 76strumenti} è illegale perché non inizia con una lettera. {\tt milione@} è illegale perché contiene un carattere non valido (\tt{@}). Ma cosa c'è di sbagliato in {\tt class}?

{\tt class} è una delle {\bf parole riservate} di Python. L'interprete usa le parole riservate per riconoscere la struttura del programma e non possono essere usate come nomi di variabili.
\index{keyword}

Python 2 ha 31 parole riservate::
...
\end{verbatim}
%
In Python 3, {\tt exec} non è più una parola riservata a keyword, ma {\tt nonlocal} lo è.
...
Gli {\bf operatori} sono simboli speciali che rappresentano elaborazioni di tipo matematico, quali la somma e la moltiplicazione. I valori che l'operatore usa nei calcoli sono chiamati {\bf operandi}.

Gli operatori {\tt +}, {\tt -}, {\tt *}, {\tt /} e {\tt **} eseguono rispettivamente le operazioni di addizione, sottrazione, moltiplicazione, divisione ed elevazione a potenza:

\begin{verbatim}
20+32   ore-1   ore*60+minuti   minuti/60   5**2   (5+9)*(15-7)
\end{verbatim}
%
In alcuni altri linguaggi, l'operatore \verb"^"" è usato per l'elevazione a potenza, ma in Python è un'operatore per bit chiamato XOR. Gli operatori per bit non saranno affrontati in questo testo, ma puoi approfondirli all'indirizzo \url{http://wiki.python.org/moin/BitwiseOperators}.
\index{bitwise operator}
\index{operator!bitwise}

In Python 2, l'operatore divisione potrebbe non fare ciò che ti aspetti:
...
\end{verbatim}
%
Il valore di {\tt minuti} è 59, e 59 diviso 60 è 0.98333, non zero. La ragione di questa differenza sta nel fatto che Python sta facendo una {\bf divisione tra numeri interi}.
Quando entrambi gli operandi sono numeri interi il risultato è sempre un numero intero e per convenzione la divisione tra numeri interi restituisce sempre un numero arrotondato all'intero inferiore ({\bf arrotondamento verso il basso}), anche nel caso in cui il risultato sia molto vicino all'intero superiore.

In Python 3, il risultato di questa divisione è un {\tt float}.  Il nuovo operatore
{\tt //} esegue la divisione con arrotondamento verso il basso.
\index{Python 3}
...
Se almeno uno degli operando è un numero a vrigola mobile, Python esegue una divisione a virgola mobile, ed il risultato è un {\tt float}:
...
\section{Espressioni ed istruzioni}

Un'{\bf espressione} è una combinazione di valori, variabili e operatori.
Un valore di per sé è considerato un'espressione, come lo è una variabile.
Le seguenti sono tutte espressioni legali (assumendo che sia stat assegnato un valore alla variabile {\tt x}):
\index{expression}
\index{evaluate}
...
\end{verbatim}
%
Un'{\bf istruzione} è un'unità di codice che l'interprete Python può eseguire. Abbiamo già visto due tipi di istruzioni: istruzioni di stampa e di assegnazione.

Tecnicamente un'espressione è anche un'istruzione, ma probabilmente è più semplice pensarle come due cose distinte. La differenza importante è che un'espressione ha un valore, mentre un'istruzione no.
